							...
							<div className="bg-secondary p-1 rounded-xl flex overflow-x-auto no-scrollbar">
								{tabs.map((t) => (
									<button
										...
										className={`px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-medium whitespace-nowrap transition-all ${activeTab === t ? "bg-card shadow-sm font-bold text-primary" : "text-muted-foreground hover:text-foreground"
											}`}
									>
										{t}
									</button>
											...
											<tr key={g.code} className={`transition-colors ${g.reported ? "bg-destructive/5 hover:bg-destructive/10" : "hover:bg-secondary/30"}`}>
												<td className="px-6 py-4">
													<div className="flex items-center gap-3">
														<div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm ${g.reported ? "bg-destructive/10 text-destructive" : g.code === "BA" ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary"
															}`}>{g.code}</div>
														<div>
							...
							{groups.map((g) => (
								<Card key={g.code} className={`border-border/50 ${g.reported ? "border-destructive/30 bg-destructive/5" : ""}`}>
									<CardContent className="p-4 space-y-3">
										<div className="flex items-center gap-3">
											<div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 ${g.reported ? "bg-destructive/10 text-destructive" : g.code === "BA" ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary"
												}`}>{g.code}</div>
