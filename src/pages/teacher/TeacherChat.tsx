		...
		<div className="flex flex-col h-full">
			...
			<div className="flex-1 overflow-y-auto px-2 space-y-0.5">
				...
				{filtered.map((c) => (
					...
					>
						<div className="relative">
							<div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold text-xs shrink-0 ${c.role === "teacher" ? "bg-stat-blue/20 text-stat-blue" : "bg-primary/15 text-primary"
								}`}>
								{getInitials(c.name)}
							</div>
						...
						<div className="flex-1 min-w-0">
							...
							<div className="flex items-center gap-1">
								<span className={`text-[9px] px-1.5 py-0.5 rounded-md font-medium ${c.role === "teacher" ? "bg-stat-blue/10 text-stat-blue" : "bg-primary/10 text-primary"
									}`}>
		...
		<div className="flex flex-col h-full">
			<header className="h-14 border-b border-border bg-card/80 backdrop-blur-md flex items-center justify-between px-4 z-10 shrink-0">
				<div className="flex items-center gap-3">
					...
					<div className={`w-9 h-9 rounded-full flex items-center justify-center font-semibold text-xs shrink-0 ${isGroup ? "bg-stat-purple/20 text-stat-purple" : "bg-primary/15 text-primary"
						}`}>
						...
						<div className="flex h-10 items-center justify-center rounded-xl bg-secondary/50 p-1">
							{(["personal", "groups"] as const).map((tab) => (
								...
								>
									...
									<span className={`min-w-4 h-4 rounded-full text-[9px] font-bold px-1 flex items-center justify-center ${activeTab === tab ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
										}`}>
