...
const TeacherSettings = () => {
	...
	const Toggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
		...
		>
			<div
				className={`absolute top-[2px] w-5 h-5 bg-card rounded-full shadow transition-transform ${checked ? "translate-x-[22px]" : "translate-x-[2px]"
					}`}
			/>
		</button>
	);
					...
					<div className="flex flex-wrap gap-3">
						{accentColors.map((c, i) => (
							<button
								...
								className={`w-8 h-8 rounded-full transition-all ${selectedAccent === i ? "ring-2 ring-offset-2 ring-offset-background scale-110" : "hover:scale-105 opacity-80 hover:opacity-100"
									}`}
								title={c.name}
							/>
						))}
